...
static inline void set_idt_gate(int n, uintptr_t handler) {
    extern void idt_set_gate(uint8 num, uintptr_t handler, uint8 flags);
    idt_set_gate((uint8)n, handler, IDT_GATE_INTERRUPT32);
}

static inline void set_idt_gate_flags(int n, uintptr_t handler, uint8 flags) {
    extern void idt_set_gate(uint8 num, uintptr_t handler, uint8 flags);
