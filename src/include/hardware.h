...
// CPUID feature bitsets from EAX=1 (ECX bits)
enum {
...
// CPUID feature bitsets from EAX=1 (EDX bits)
enum {
...
typedef struct {
    ...
    uint32 extended_ecx;
    uint32 extended_edx;
    uint32 structured_ebx;
    uint32 structured_ecx;
} cpuid_feature_bits_t;
...
typedef struct {
    ...
    uint8 model;
    uint8 stepping;
    uint8 extended_family;
    uint8 extended_model;
    uint8 apic_id;
    uint8 logical_processor_count;
    bool hypervisor_present;
    cpuid_feature_bits_t features;
    char feature_summary[192];
} cpuid_info_t;

// Initialize CPUID detection. Returns true if CPUID is supported.
bool hardware_detect_init(void);

// Returns cached CPUID information.
const cpuid_info_t* hardware_get_cpuid_info(void);

// Returns a summary string with key CPU features.
